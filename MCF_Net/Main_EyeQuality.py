...
def generate_detailed_report(gt_labels, predictions, metrics_dict, args, best_epoch, best_loss, class_names):
    ...
    report.append(f"Precision (micro): {metrics_dict['micro-Precision'].item():.4f}")
    report.append(f"Sensitivity (micro): {metrics_dict['micro-Sensitivity'].item():.4f}")
    report.append(f"Specificity (micro): {metrics_dict['micro-Specificity'].item():.4f}")
    report.append(f"F1 (micro): {metrics_dict['micro-F1'].item():.4f}")
    report.append(f"AUC: {metrics_dict['AUC']:.4f}")
    
    report.append("\nPer-class F1 scores:")
    for i, class_name in enumerate(class_names):
        report.append(f"  {class_name}: {metrics_dict['F1'][i].item():.4f}")
...
if __name__ == '__main__':
    ...
    if args.pre_model is not None:
        loaded_model = torch.load(os.path.join(args.model_dir, args.pre_model + '.tar'), weights_only=False)
    ...
    loaded_model = torch.load(os.path.join(args.save_dir, args.save_model + '.tar'), weights_only=False)
    model.load_state_dict(loaded_model['state_dict'])
    model.to(device)
